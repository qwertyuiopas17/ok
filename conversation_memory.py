...
class ProgressiveConversationMemory:
    ...
    def show_appointment_button(self, user_id: str, show: bool = True) -> None:
        """Show/hide appointment booking button"""
        profile = self.create_or_get_user(user_id)
        profile.show_appointment_button = show
        if show:
            profile.pending_actions.append('book_appointment')

    def show_medicine_scan_button(self, user_id: str, show: bool = True) -> None:
        """Show/hide medicine scan button"""
        profile = self.create_or_get_user(user_id)
        profile.show_medicine_scan_button = show
        if show:
            profile.pending_actions.append('scan_medicine')

    def show_prescription_button(self, user_id: str, show: bool = True) -> None:
        """Show/hide prescription button"""
        profile = self.create_or_get_user(user_id)
        profile.show_prescription_button = show
        if show:
            profile.pending_actions.append('view_prescription')
    ...
    def add_prescription_summary(self, user_id: str, prescription_data: Dict[str, Any]) -> None:
        """Add prescription summary for user and auto-generate medicine reminders"""
        profile = self.create_or_get_user(user_id)

        prescription_id = prescription_data.get('prescription_id', f'rx_{datetime.now().isoformat()}')
        profile.prescription_summary[prescription_id] = {
            'doctor_name': prescription_data.get('doctor_name', ''),
            'medications': prescription_data.get('medications', []),
            'diagnosis': prescription_data.get('diagnosis', ''),
            'instructions': prescription_data.get('instructions', ''),
            'date': datetime.now().isoformat(),
            'summary': self._summarize_prescription(prescription_data)
        }

        # Auto-generate medicine reminders from prescription
        if prescription_data.get('medications'):
            self._auto_generate_medicine_reminders(user_id, prescription_data)

        self.logger.info(f"Added prescription summary for user {user_id}")

    def _auto_generate_medicine_reminders(self, user_id: str, prescription_data: Dict[str, Any]) -> None:
        """Auto-generate medicine reminders from prescription data"""
        profile = self.create_or_get_user(user_id)

        if 'medicine_reminders' not in profile.user_data:
            profile.user_data['medicine_reminders'] = []

        medications = prescription_data.get('medications', [])
        doctor_name = prescription_data.get('doctor_name', 'Doctor')

        for medication in medications:
            # Skip if reminder already exists for this medicine
            existing_reminder = None
            for reminder in profile.user_data['medicine_reminders']:
                if reminder['medicine_name'].lower() == medication['name'].lower():
                    existing_reminder = reminder
                    break

            if existing_reminder:
                # Update existing reminder with prescription data
                existing_reminder.update({
                    'dosage': medication.get('dosage', existing_reminder.get('dosage', '')),
                    'instructions': medication.get('instructions', existing_reminder.get('instructions', '')),
                    'source': 'prescription',
                    'doctor_name': doctor_name,
                    'last_updated': datetime.now().isoformat()
                })
            else:
                # Create new reminder from prescription
                # Infer timing from dosage instructions
                times = self._infer_medicine_timing(medication.get('dosage', ''))
                ...
                reminder = {
                    ...
                    'times': times,
                    'duration_days': medication.get('duration_days', 30),
                ...
                profile.user_data['medicine_reminders'].append(reminder)

    def _infer_medicine_timing(self, dosage_text: str) -> List[str]:
        """Infer medicine timing from dosage instructions"""
        dosage_lower = dosage_text.lower()

        # Common timing patterns
        if any(word in dosage_lower for word in ['thrice', 'three times', '3 times', 'तीन बार', 'तिन पटक']):
            return ['08:00', '14:00', '20:00']
        elif any(word in dosage_lower for word in ['twice', 'two times', '2 times', 'दो बार', 'दु पटक']):
            return ['08:00', '20:00']
        elif any(word in dosage_lower for word in ['four times', '4 times', 'चार बार']):
            return ['06:00', '10:00', '14:00', '18:00', '22:00']
        elif any(word in dosage_lower for word in ['morning', 'सुबह', 'ਸਵੇਰੇ']):
            return ['08:00']
        elif any(word in dosage_lower for word in ['evening', 'night', 'शाम', 'ਰਾਤ']):
            return ['20:00']
        elif any(word in dosage_lower for word in ['after food', 'after meals', 'खाने के बाद', 'ਭੋਜਨ ਤੋਂ ਬਾਅਦ']):
            return ['08:30', '13:30', '20:30']
        elif any(word in dosage_lower for word in ['before food', 'before meals', 'खाने से पहले', 'ਭੋਜਨ ਤੋਂ ਪਹਿਲਾਂ']):
            return ['07:30', '12:30', '19:30']
        else:
            # Default timing
            return ['08:00', '20:00']

    def _summarize_prescription(self, prescription_data: Dict[str, Any]) -> str:
        """Create a summarized version of prescription"""
        medications = prescription_data.get('medications', [])
        diagnosis = prescription_data.get('diagnosis', '')
        ...
        for med in medications[:3]:  # Limit to first 3 medications
            ...
            summary.append(f"{name} {dosage} {frequency}".strip())

        result = ", ".join(summary)
        if len(medications) > 3:
            result += f" and {len(medications) - 3} more medications"

        if diagnosis:
            result = f"Diagnosis: {diagnosis}. Medications: {result}"

        return result

    def get_prescription_summary(self, user_id: str, prescription_id: str = None) -> Dict[str, Any]:
        ...
        if prescription_id:
            return profile.prescription_summary.get(prescription_id, {})
        else:
            # Return latest prescription
            if profile.prescription_summary:
                latest_id = max(profile.prescription_summary.keys(),
                              key=lambda x: profile.prescription_summary[x].get('date', ''))
                return profile.prescription_summary[latest_id]
    ...
    def schedule_medicine_reminder(self, user_id: str, medicine_data: Dict[str, Any]) -> None:
        ...
        reminder = {
            ...
            'times': medicine_data['times'],  # ['08:00', '14:00', '20:00']
        ...
        profile.user_data['medicine_reminders'].append(reminder)
        self.save_to_file('conversation_memory.json')
    ...
    def get_medicine_reminders(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        active_reminders = []
        for reminder in profile.user_data['medicine_reminders']:
            if reminder['reminder_enabled']:
                active_reminders.append(reminder)

        return active_reminders

    def add_doctor_prescription_summary(self, user_id: str, appointment_data: Dict[str, Any]) -> None:
        """Add prescription summary from doctor's appointment and auto-generate reminders"""
        if not appointment_data.get('prescription'):
            return

        profile = self.create_or_get_user(user_id)

        if 'prescription_summary' not in profile.user_data:
            profile.user_data['prescription_summary'] = {}

        # Create prescription data from appointment
        prescription_data = {
            'prescription_id': f"apt_{appointment_data.get('appointment_id', 'unknown')}",
            'doctor_name': appointment_data.get('doctor_name', 'Doctor'),
            'medications': appointment_data['prescription'].get('medications', []),
            'diagnosis': appointment_data['prescription'].get('diagnosis', ''),
            'instructions': appointment_data['prescription'].get('instructions', ''),
            'source': 'doctor_appointment',  # Mark as doctor-provided prescription
            'appointment_id': appointment_data.get('appointment_id'),
            'appointment_date': appointment_data.get('appointment_date'),
            'consultation_mode': appointment_data.get('consultation_mode', 'unknown')
        }

        # Add to prescription summary
        profile.user_data['prescription_summary'][prescription_data['prescription_id']] = prescription_data

        # Auto-generate medicine reminders from doctor's prescription
        if prescription_data.get('medications'):
            self._auto_generate_medicine_reminders(user_id, prescription_data)

        self.save_to_file('conversation_memory.json')
        self.logger.info(f"Added doctor prescription summary for user {user_id} from appointment {appointment_data.get('appointment_id')}")
    ...
    def update_reminder_adherence(self, user_id: str, medicine_name: str, taken_time: str) -> None:
        ...
        if 'medicine_adherence' not in profile.user_data:
            profile.user_data['medicine_adherence'] = {}

        today = datetime.now().strftime('%Y-%m-%d')
        if today not in profile.user_data['medicine_adherence']:
            profile.user_data['medicine_adherence'][today] = {}

        profile.user_data['medicine_adherence'][today][medicine_name] = {
    ...
    def get_reminder_alerts(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        for reminder in reminders:
            # Check if reminder time has passed and not taken today
            for time_slot in reminder['times']:
                if time_slot <= current_time:
                    # Check if already taken today
                    profile = self.get_user_profile(user_id)
                    adherence = {}
                    if profile and 'medicine_adherence' in profile.user_data:
                        adherence = profile.user_data['medicine_adherence'].get(today, {})

                    if reminder['medicine_name'] not in adherence or adherence[reminder['medicine_name']]['status'] != 'taken':
                        alerts.append({
    ...
    def get_user_progress_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        # Get prescription sources
        prescription_sources = {}
        total_prescriptions = len(profile.prescription_summary)

        for prescription_id, prescription_data in profile.prescription_summary.items():
            source = prescription_data.get('source', 'unknown')
            if source not in prescription_sources:
                prescription_sources[source] = 0
            prescription_sources[source] += 1

        return {
            'appointment_history': profile.appointment_status,
            'prescription_count': total_prescriptions,
            'prescription_sources': prescription_sources,
            'last_appointment': profile.last_appointment_date.isoformat() if profile.last_appointment_date else None,
            'appointments_booked': profile.total_appointments_booked,
            'appointments_completed': profile.appointments_completed,
            'feedback_pending': profile.post_appointment_feedback_pending,
            ...
            'pending_actions': profile.pending_actions,
            'medicine_reminders_count': len(profile.user_data.get('medicine_reminders', [])),
            'prescription_summary_available': total_prescriptions > 0
        }
